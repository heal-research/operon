...
auto InitOptions(std::string const& name, std::string const& desc, int width) -> cxxopts::Options
{
    ...
    opts.add_options()
        ...
        ("sigma", "Expected sigma error", cxxopts::value<Operon::Scalar>()->default_value("1.0"))
        ...
        ("use-transposition-aware-crossover", "Prevents crossover from creating already seen solutions", cxxopts::value<bool>()->default_value("false"))
        ("use-transposition-aware-mutation", "Prevents mutation from creating already seen soutions", cxxopts::value<bool>()->default_value("false"))
