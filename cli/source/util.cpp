...
auto InitOptions(std::string const& name, std::string const& desc, int width) -> cxxopts::Options
{
    ...
    opts.add_options()
        ...
        ("creator", "Tree creator operator, with optional parameters separated by : (name:bias:mindepth:maxdepth:maxlength, e.g. --creator ptc2:0.5::20:30 - any parameter left empty or omitted falls back to --creator-mindepth/--creator-maxdepth/--maxlength)", cxxopts::value<std::string>()->default_value("btc"))
        ("creator-mindepth", "Default minimum tree depth for --creator, when not overridden there (applies to the grow tree creator)", cxxopts::value<std::size_t>()->default_value("1"))
        ("creator-maxdepth", "Default maximum tree depth for --creator, when not overridden there (applies to all tree creators)", cxxopts::value<std::size_t>()->default_value("100"))
        ...
        ("mutators", "Comma-separated mutation operators to use, with optional weight separated by : (default weight 1.0), e.g. --mutators onepoint:2,changefunc,replacesubtree. Available: onepoint, multipoint, changevar, changefunc, replacesubtree, insertsubtree, removesubtree, discretepoint, shuffle", cxxopts::value<std::string>()->default_value("onepoint:1,changevar:1,changefunc:1,replacesubtree:1,insertsubtree:1,removesubtree:1,discretepoint:1"))
