...
auto InitOptions(std::string const& name, std::string const& desc, int width) -> cxxopts::Options
{
    ...
    opts.add_options()
        ...
        ("skip-nonfinite", "Skip non-finite rows instead of clamping fitness to ErrMax on any non-finite prediction (MSE/NMSE/RMSE/MAE objectives only)", cxxopts::value<bool>()->default_value("false"))
        ...
        ("cache-max-age", "Treat revisited transposition-cache entries older than this many generations as misses (0 = never expire); controls staleness, not total cache memory", cxxopts::value<size_t>()->default_value("0"))
