...
auto InitOptions(std::string const& name, std::string const& desc, int width) -> cxxopts::Options
{
    ...
    opts.add_options()
        ...
        ("creator", "Tree creator: name:bias:mindepth:maxdepth:maxlength (e.g. --creator ptc2:0.5::20:30). Omitted fields use --creator-mindepth/--creator-maxdepth/--maxlength. BTC treats depths as advisory to preserve length; PTC2 enforces max depth but can return shorter trees.", cxxopts::value<std::string>()->default_value("btc"))
        ("creator-mindepth", "Default minimum tree depth for --creator; honored by grow, advisory for BTC/PTC2", cxxopts::value<std::size_t>()->default_value("1"))
        ("creator-maxdepth", "Default maximum tree depth for --creator; enforced by grow/PTC2, advisory for BTC", cxxopts::value<std::size_t>()->default_value("100"))
