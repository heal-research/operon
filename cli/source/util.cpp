...
auto InitOptions(std::string const& name, std::string const& desc, int width) -> cxxopts::Options
{
    ...
    opts.add_options()
        ...
        ("shape-bound-mode", "Comma-separated shape-constraint bound backend flags: combined (default), interval, affine, bisected (only with interval)", cxxopts::value<std::string>()->default_value("combined"))
