...
auto InitOptions(std::string const& name, std::string const& desc, int width) -> cxxopts::Options
{
    ...
    opts.add_options()
        ...
        ("shape-bound-mode", "Arithmetic backend for shape-constraint bound computation: combined (affine intersected with interval, default), interval-only, affine-only, interval-only-bisected (recursive interval-only domain bisection, no affine)", cxxopts::value<std::string>()->default_value("combined"))
