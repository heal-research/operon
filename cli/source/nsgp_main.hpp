...
// Returns the Pareto-front (Rank==0) individual with minimum MDL score.
...
auto nsgp_main(int argc, char** argv) -> int
{
    auto opts = Operon::InitOptions("operon_gp", "Genetic programming symbolic regression");
    opts.add_options()
        ("model-selection", "Pareto front model selection: mse or mdl",
         cxxopts::value<std::string>()->default_value("mse"))
        ("mdl-likelihood", "Likelihood for MDL model selection: gaussian or poisson",
    ...
    try {
        for (const auto& kv : result.arguments()) {
            ...
            if (key == "seed")         { config.Seed = kv.as<size_t>(); }
            if (key == "train")        { trainingRange = Operon::ParseRange(value); }
            if (key == "test")         { testRange = Operon::ParseRange(value); }
            if (key == "target")       { targetName = value; }
            if (key == "maxlength")    { maxLength = kv.as<size_t>(); }
            if (key == "maxdepth")     { maxDepth = kv.as<size_t>(); }
            if (key == "enable-symbols")  { primitiveSetConfig |= Operon::ParsePrimitiveSetConfig(value); }
            if (key == "disable-symbols") { primitiveSetConfig &= ~Operon::ParsePrimitiveSetConfig(value); }
            if (key == "threads")      { threads = static_cast<decltype(threads)>(kv.as<size_t>()); }
            if (key == "show-primitives") { showPrimitiveSet = true; }
        }
        ...
        if (result.count("train") == 0) {
            trainingRange = Operon::Range { 0, 2 * rows / 3 };
        }
        ...
        if (result.count("inputs") == 0) {
            ...
            for (auto const& tok : tokens) {
                if (auto v = dataset->GetVariable(tok); v.has_value()) {
        ...
        treeInitializer.ParameterizeDistribution(amin + 1, result["creator-maxlength"].as<std::size_t>());
        treeInitializer.SetMinDepth(result["creator-mindepth"].as<std::size_t>());
        treeInitializer.SetMaxDepth(result["creator-maxdepth"].as<std::size_t>());
        ...
        for (auto v : Operon::Math::Constants) { discretePoint.Add(static_cast<Operon::Scalar>(v), 1); }
        ...
        AlgorithmType gp { config, &problem, &treeInitializer, coeffInitializer.get(), generator.get(), reinserter.get(), sorterPtr };

        Operon::ModelSelectorFn modelSelector;
        if (result["model-selection"].as<std::string>() == "mdl") {
