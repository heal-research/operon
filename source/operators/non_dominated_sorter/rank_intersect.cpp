...
namespace detail {
    ...
    template<typename Item, typename Rank, typename RankSet>
    inline auto UpdateRanks(Item const& item, Rank& rank, RankSet& rankset)
    {
        ...
        for (std::size_t j = o; j < nb; ++j) {          // iterate over bitset blocks
            ...
            for (; x != 0; x &= (x - 1)) {              // iterate over set bits of v
                auto k = j * D + CountTrailingZeros(x); // get index of dominated individual
...
auto RankIntersectSorter::Sort(Operon::Span<Operon::Individual const> pop, Operon::Scalar /*unused*/) const -> NondominatedSorterBase::Result
{
    ...
    for (auto obj = 1; obj < m; ++obj) {
        ...
        std::stable_sort(items.begin(), items.end(), [](auto const& a, auto const& b) { return std::get<2>(a) < std::get<2>(b); });
