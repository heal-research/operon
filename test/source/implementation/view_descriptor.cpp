...
TEST_CASE("view_descriptor: MakeMatrixView writes back through the borrowed pointer", "[view-descriptor]")
{
    ...
    auto view = Operon::MakeMatrixView(desc);
    REQUIRE(view.has_value());
...
TEST_CASE("view_descriptor: MakeMatrixView rejects a read-only descriptor", "[view-descriptor]")
{
    ...
    auto mutableView = Operon::MakeMatrixView(desc);
...
TEST_CASE("view_descriptor: operon_view_validate rejects malformed descriptors", "[view-descriptor]")
{
    ...
    SECTION("zero extent")
    {
        auto desc = MakeValidDescriptor(storage);
        desc.extents[1] = 0;
        CHECK(operon_view_validate(&desc, 0) == OPERON_VIEW_ERR_EXTENT);
    }
