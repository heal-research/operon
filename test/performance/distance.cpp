...
TEST_CASE("Intersection performance") 
{
    ...
    std::transform(trees.begin(), trees.end(), hashesStrict.begin(), [](Tree tree) { return MakeHashes(tree, Operon::HashMode::Strict); });
    std::transform(trees.begin(), trees.end(), hashesStruct.begin(), [](Tree tree) { return MakeHashes(tree, Operon::HashMode::Relaxed); });
    ...
    SUBCASE("Performance") {
        ...
        b.run("intersect strict (simd)", [&](){ ankerl::nanobench::doNotOptimizeAway(Operon::Distance::CountIntersectSIMD(hashesStrict[dist(rd)], hashesStrict[dist(rd)])); });
        b.run("intersect struct (simd)", [&](){ ankerl::nanobench::doNotOptimizeAway(Operon::Distance::CountIntersectSIMD(hashesStruct[dist(rd)], hashesStruct[dist(rd)])); });
