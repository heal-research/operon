...
namespace Operon::Backend {
    ...
    using Map = Fastor::TensorMap<T, S>;

    template<typename T, std::size_t S>
    auto Col(Backend::View<T, S> view, std::integral auto col) {
        using U = std::remove_const_t<T>;
        using M = std::conditional_t<std::is_const_v<T>, Map<U, S> const, Map<T, S>>;
        return M(const_cast<U*>(view.data_handle() + col * S));
    }
    ...
    auto Sub(T* res, auto* first, auto*... rest) {
        ...
        Map<T, S>{res} = Map<T, S>{first} - (Map<T, S>{rest} + ...);
    }
    ...
    auto Min(T* res, auto* first, auto*... args) {
        ...
        Map<T, S>{res} = (Fastor::min(Map<T, S>{first}, Map<T, S>{args}), ...);
    }

    template<typename T, std::size_t S>
    auto Max(T* res, auto* first, auto*... args) {
        static_assert(sizeof...(args) > 0);
        Map<T, S>{res} = (Fastor::max(Map<T, S>{first}, Map<T, S>{args}), ...);
    }
    ...
    auto Pow(T* res, T* a, T* b) {
        Map<T, S>{res} = Fastor::pow(Map<T, S>{a}, Map<T, S>{b});
    }
