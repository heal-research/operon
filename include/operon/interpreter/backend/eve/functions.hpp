...
namespace Operon::Backend {
    ...
    auto Min(T* res, auto*... args) {
        ...
        constexpr auto L = W::size();
        for (auto i = 0UL; i < S; i += L) {
            eve::store(eve::min(W{args+i}...), res+i);
        }
    }
    ...
    auto Max(T* res, auto*... args) {
        ...
        constexpr auto L = W::size();
        for (auto i = 0UL; i < S; i += L) {
            eve::store(eve::max(W{args+i}...), res+i);
        }
    }
    ...
    auto Aq(T* res, T* a, T* b) {
        ...
        constexpr auto L = W::size();
        for (auto i = 0UL; i < S; i += L) {
            eve::store(W{a+i} / eve::sqrt(T{1} + eve::sqr(W{b+i})), res+i);
        }
    }
    ...
    auto Pow(T* res, T* a, T* b) {
        ...
        constexpr auto L = W::size();
        for (auto i = 0UL; i < S; i += L) {
            eve::store(eve::pow(W{a+i}, W{b+i}), res+i);
        }
    }
    ...
    auto Sqrtabs(T* res, T* arg) {
        eve::algo::transform_to(std::span{arg, S}, res, [](auto w) {
            return eve::sqrt(eve::abs(w));
        });
    }
