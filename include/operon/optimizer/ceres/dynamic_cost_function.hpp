    ...
    auto Evaluate(double const* const* parameters, double* residuals, double** jacobians) const -> bool override
    {
        ...
        if constexpr (std::is_same_v<Scalar, double>) {
            ...
            if (jacobians == nullptr) {
                ...
                // JacRev always writes column-major; jMap uses row-major because
                // Ceres expects row-major Jacobians. Eigen's assignment handles the conversion.
                Eigen::Matrix<Scalar, -1, -1> jacob(nr, np);
                auto success = cf_(param.data(), resid.data(), jacob.data());
                if (!success) {
                    return false;
                }

                Eigen::Map<Eigen::Matrix<double, -1, -1, Eigen::RowMajor>> jMap(jacobians[0], nr, np);
