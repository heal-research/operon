...
namespace Operon {
    ...
    namespace
    {
        template<bool IsConst, typename T = std::conditional_t<IsConst, Tree const, Tree>, typename U = std::conditional_t<IsConst, Node const, Node>> class ChildIteratorImpl 
        {
            public:
                ...
                explicit ChildIteratorImpl(T& tree, gsl::index i) : nodes(tree.Nodes()), parentIndex(i), index(i-1), count(0), arity(nodes[i].Arity) { }
                ...
                {
                    return &nodes.data() == &rhs.nodes.data() && 
    ...
    class Tree
    {
        public:
            ...
            inline void SetEnabled(gsl::index i, bool enabled) { for (int j = i - nodes[i].Length; j <= i; ++j) { nodes[j].IsEnabled = enabled; } }
