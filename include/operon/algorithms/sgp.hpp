...
namespace Operon 
{
    ...
    void GeneticAlgorithm(operon::rand_t& random, const Problem& problem, const GeneticAlgorithmConfig config, TCreator& creator, TRecombinator& recombinator)
    {
        ...
        for (size_t gen = 0; gen < config.Generations; ++gen)
        {
            ...
            fmt::print("{:#3.3f}\t{}\t{:.1f}\t{:.3f}\t{:.3f}\t{:.4f}\t{}\t{}\t{}\t{:.4f}\t{:.4f}\n", elapsed, gen+1, avgLength, hybridDiversity, structDiversity, avgQuality, evaluator.FitnessEvaluations(), evaluator.LocalEvaluations(), evaluator.TotalEvaluations(), errorTrain, errorTest);
