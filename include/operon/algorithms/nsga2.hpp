...
public:
    ...
    auto Run(Operon::RandomGenerator& random, std::function<void()> report = nullptr, size_t threads = 0) -> void
    {
        if (threads == 0U) {
    ...
    auto Run(tf::Executor& executor, Operon::RandomGenerator& random, std::function<void()> report = nullptr) -> void;
};
} // namespace Operon
