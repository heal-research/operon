...
struct OPERON_EXPORT CreatorBase : public OperatorBase<Tree, size_t, size_t, size_t> {
    CreatorBase(gsl::not_null<PrimitiveSet const*> pset, std::vector<Operon::Hash> variables)
        : pset_(pset)
    ...
    void SetPrimitiveSet(gsl::not_null<PrimitiveSet const*> pset) { pset_ = pset; }
...
public:
    BalancedTreeCreator(gsl::not_null<PrimitiveSet const*> pset, std::vector<Operon::Hash> variables, double bias = 0.0)
...
class OPERON_EXPORT GrowTreeCreator final : public CreatorBase {
    public:
        GrowTreeCreator(gsl::not_null<PrimitiveSet const*> pset, std::vector<Operon::Hash> variables)
            : CreatorBase(pset, std::move(variables))
        { }
...
public:
    ProbabilisticTreeCreator(gsl::not_null<PrimitiveSet const*> pset, std::vector<Operon::Hash> variables, double bias = 0.0)
        : CreatorBase(pset, std::move(variables))
        , irregularityBias_(bias)
    {
    }
